...
export const getFilteredTodos = (initialTodos: Todo[], query: IQuery) => {
  ...
  return initialTodos.filter(todo => {
    const isMatchingStatus =
      status === SortField.Active ? !todo.completed : status === SortField.Completed ? todo.completed : true;
    const isMatchingSearchQuery = !searchQuery || todo.title.toLowerCase().includes(searchQuery.toLowerCase());
