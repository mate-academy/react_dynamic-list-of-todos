...
## Description
You are given a basic markup for the App, TodosList and User components and [the API](https://mate-academy.github.io/fe-students-api/).
...
1. `User` component receives `userId` as a prop and loads user details from [GET user endpoint](https://mate.academy/students-api/users/1) (replace 1 with a given `userId`).
...
1. Add a button `Clear` into the `User` to clear the selectedUser in the `App`
