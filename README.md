...
- Replace `<your_account>` with your Github username in the
  [DEMO LINK](https://Paavlo.github.io/react_dynamic-list-of-todos/)
...
1. Todos are fetched on page load from [GET todosPublic endpoint](https://mate-api.herokuapp.com/todos). (Use `componentDidMount`)
...
1. Add an `<input>` to the `TodoList` to filter the todosPublic by title
1. Add a `<select>` to the `TodoList` to show `all`, `active`(not completed) or `completed` todosPublic.
1. (*) Add `Randomize` button to the `TodoList` to show the todosPublic in a random order.
